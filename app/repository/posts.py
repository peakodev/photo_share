...
async def get_posts(limit: int, offset: int, user: User, db: AsyncSession) -> List[Post]:
    ...
    query = select(Post).filter_by(user=user).options(selectinload(Post.comments), selectinload(Post.tags)).offset(offset).limit(limit)
...
async def get_all_posts(limit: int, offset: int, db: AsyncSession) -> List[Post]:
    ...
    query = select(Post).options(selectinload(Post.comments), selectinload(Post.tags)).offset(offset).limit(limit)
    posts = await db.execute(query)
    return posts.scalars().all()
...
async def get_post(post_id: int, user: User, db: AsyncSession) -> Post | None:
    ...
    query = select(Post).filter_by(id=post_id, user=user).options(selectinload(Post.comments), selectinload(Post.tags))
...
async def create_post(url: str, description, tags, 
    ...
    query = select(Post).filter_by(id=new_post.id, user=user).options(selectinload(Post.tags))
