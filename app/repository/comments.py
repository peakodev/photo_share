...
async def get_comments_by_post(post_id: int, offset: int, limit: int, db: AsyncSession) -> List[Comment] | None:
    query = select(Comment).filter_by(post_id = post_id).group_by(Comment.created_at, Comment.id).offset(offset).limit(limit)
    comments = await db.execute(query)
    return comments.scalars().all()


async def create_comment(comment: CommentCreate, user_id: int,  db: AsyncSession) -> Comment:
...
async def update_comment(comment_id: int, comment: CommentUpdate, db: AsyncSession) -> Comment | None:
    query = select(Comment).filter_by(id=comment_id)
